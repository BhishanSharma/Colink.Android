...
public class SenderLoadingActivity extends AppCompatActivity implements FileClientService.GetSentFile {
    private final ArrayList<File> sentFiles = new ArrayList<>();
    ...
    private final ServiceConnection serviceConnection = new ServiceConnection() {
        ...
        public void onServiceConnected(ComponentName name, IBinder service) {
            try {
                ...
                fileClientService.setGetSentFileListener(SenderLoadingActivity.this);
                isBound = true;
            } catch (Exception e) {
                Toast.makeText(SenderLoadingActivity.this, "Error in onServiceConnected: " + e.getMessage(), Toast.LENGTH_LONG).show();
            }
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_sender_loading);
        try {
    ...
    private void updateUI() {
        mainHandler.post(() -> {
            try {
                updateFileCountText();
                if (fileCount > 0) {
                    int progress = (sentFiles.size() * 100) / fileCount;
    ...
    private void updateFileCountText() {
        try {
            fileCountTV.setText(String.format("%d/%d", sentFiles.size(), fileCount));
        } catch (Exception e) {
    ...
    private void displayFiles() {
        try {
            if (!sentFiles.isEmpty()) {
                adapter.updateData(convertListToMap(sentFiles));
            } else {
